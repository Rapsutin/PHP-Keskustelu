...
<div class="container">
    ...
    <form action ="muokkaaAihetta.php?aiheID=<?php echo $muokattavanID; ?>" method ="POST">
        ...
        <div>
            Aiheen nimi
            <input type="text" style="width: 240px" class="form-control" name="aiheenNimi" 
                   value="<?php
                   if (isset($data->keskenerainenNimi)) {
                       echo $data->keskenerainenNimi;
                   } else {
                       echo $muokattavaAihe->getNimi();
                   }
                   ?>"></input>
        </div>
